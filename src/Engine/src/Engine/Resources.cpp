...
auto engine::LoaderTexture::load(const std::string_view path, std::array<float, 4ul> &&clip) const -> std::shared_ptr<Texture>
{
    return std::shared_ptr<Texture>(new Texture{Texture::ctor(path, std::move(clip))}, Texture::dtor);
}
