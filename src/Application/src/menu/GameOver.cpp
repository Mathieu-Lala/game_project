...
void game::menu::GameOver::create(entt::registry &, ThePURGE &)
{
    ...
    m_backgroundTexture = engine::helper::loadTexture(dataFolder + "textures/menu/game_over/background.png");

    // clang-format off

    // SAME ORDER AS `Button` ENUM
    m_buttons.emplace_back(GUITexture{
        helper::getTexture("textures/menu/game_over/btn_playagain_selected.png"),
        helper::from1080p(701, 665),
        helper::from1080p(515, 156)
    });
    m_buttons.emplace_back(GUITexture{
        helper::getTexture("textures/menu/game_over/btn_menu_selected.png"),
